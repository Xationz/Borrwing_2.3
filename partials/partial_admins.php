<?php
// partial_admins.php — SPA partial (content only)
...
if (isset($_POST['add_admin'])) {
    ...
    if (empty($uname) || empty($pass)) { $swal_msg = "Swal.fire('ผิดพลาด','กรุณากรอก Username และ Password','error');"; }
        ...
        try {
            $pdo->prepare("INSERT INTO users (username, password, role) VALUES (?, ?, 'admin')")->execute([$uname, password_hash($pass, PASSWORD_BCRYPT)]);
...
if (isset($_POST['update_admin'])) {
    $id = (int)$_POST['admin_id']; $uname = trim($_POST['username']); $pass = trim($_POST['password']);
    if (empty($uname)) { $swal_msg = "Swal.fire('ผิดพลาด','กรุณากรอก Username','error');"; }
    else {
        try {
            if ($pass) { $pdo->prepare("UPDATE users SET username=?, password=? WHERE id=? AND role='admin'")->execute([$uname, password_hash($pass, PASSWORD_BCRYPT), $id]); }
            else       { $pdo->prepare("UPDATE users SET username=? WHERE id=? AND role='admin'")->execute([$uname, $id]); }
        ...
        <form method="POST">
            <div class="row g-3">
                <div class="col-md-5">
                    ...
                    <input type="text" name="username" class="form-control" placeholder="กรอก Username" required>
                </div>
                <div class="col-md-5">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <a href="?page=admins&delete_admin=<?php echo $admin['id']; ?>" class="btn btn-danger btn-sm spa-confirm-delete"
                           data-confirm="ยืนยันการลบแอดมินนี้?">
